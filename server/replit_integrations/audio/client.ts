...
/** Convert any audio/video container to 16kHz mono WAV via ffmpeg. */
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      const ffmpeg = spawn("ffmpeg", [
        "-i", inputPath,
        "-vn", "-f", "wav", "-ar", "16000", "-ac", "1", "-acodec", "pcm_s16le",
        "-y", outputPath,
      ]);
 ...
 * - WAV/MP3 pass through, anything else is converted to WAV via ffmpeg.
 */
